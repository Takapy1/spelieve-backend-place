...
export class PBL002UpsertPlaceDataService {
  ...
  async doExecute(body: UpsertPlaceDataBodyInterface) {
    ...
    const placeDocumentSnap: QueryDocumentSnapshot | null =
      await placeCollectionRef
    ...
    const photoUrls: string[] = await Promise.all(
      googlePlaceDetailsResult.photos
        .slice(0, NumberOfImagesPlaceStores)
        .map(async (photo) => {
    ...
    const mPlace: MPlace = {
      ...
      locality: googlePlaceDetailsResult.address_components.find(
        (address_component) => address_component.types.includes('locality'),
      )?.long_name,
      sublocalityLevel1: googlePlaceDetailsResult.address_components.find(
        (address_component) =>
          address_component.types.includes('sublocalityLevel1'),
      )?.long_name,
      sublocalityLevel2: googlePlaceDetailsResult.address_components.find(
        (address_component) =>
          address_component.types.includes('sublocalityLevel2'),
      )?.long_name,
      sublocalityLevel3: googlePlaceDetailsResult.address_components.find(
        (address_component) =>
          address_component.types.includes('sublocalityLevel3'),
      )?.long_name,
      sublocalityLevel4: googlePlaceDetailsResult.address_components.find(
        (address_component) =>
          address_component.types.includes('sublocalityLevel4'),
      )?.long_name,
      premise: googlePlaceDetailsResult.address_components.find(
        (address_component) => address_component.types.includes('premise'),
      )?.long_name,
      ...
      createdAt: placeDocumentSnap
        ? placeDocumentSnap.get(MPlace.Cols.createdAt)
        : Timestamp.now(),
